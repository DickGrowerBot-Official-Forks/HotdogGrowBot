...
use crate::domain::primitives::{AttemptsCount, Count, Limit, ScheduledBroadcastId, SupportedLanguage};
...
pub struct ScheduledBroadcast {
    ...
    /// The row's own foreign key, which the shrinks are stored under. Carried alongside the
...
repository!(ScheduledBroadcasts,
    ...
    pub async fn claim_due(&self, limit: Limit, lease_until: DateTime<Utc>) -> anyhow::Result<Vec<ScheduledBroadcast>> {
        ...
        for row in rows {
            let Some(chat_id) = row.chat_id else {
                ...
                self.finish(row.id, BroadcastState::Failed, None).await
    ...
    ///
    /// `lang` rides along rather than travelling in a statement of its own: the row is being
    /// written anyway, and the point of storing the language is to save the next attempt a lookup,
    /// which an extra round trip here would have spent in advance. `NULL` leaves what is stored
    /// alone, so a caller that never worked the language out cannot erase one that did.
    #[autometrics]
    ...
    ) -> anyhow::Result<AttemptsCount> {
        let attempts = sqlx::query_scalar!(
            r#"UPDATE Scheduled_Shrink_Broadcasts
                    SET attempts = attempts + 1, fire_after = $2, lang_code = COALESCE($3, lang_code)
                    WHERE id = $1 RETURNING attempts AS "attempts!: AttemptsCount""#,
                id as ScheduledBroadcastId, retry_after, lang as Option<SupportedLanguage>)
            .fetch_one(&self.pool)
            .await
    ...
    /// takes it away. `lang` behaves as in [`Self::postpone`].
    #[autometrics]
    ...
    pub async fn finish(
        ...
        sqlx::query!(
            "UPDATE Scheduled_Shrink_Broadcasts
                    SET state = $2, finished_at = current_timestamp, lang_code = COALESCE($3, lang_code)
                    WHERE id = $1",
                id as ScheduledBroadcastId, state as BroadcastState, lang as Option<SupportedLanguage>)
            .execute(&self.pool)
            .await
